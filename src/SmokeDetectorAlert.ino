...
int lastButtonState = HIGH;   // previous state of the button (startup high = no difference between buttonState)
...
void loop() {
  client.loop();
  reconnect();
  heartbeat("home/smokedetector/heartbeat","on");
  handle_alerts();
  mqttloop();
}

void heartbeat(const char * topic, const char * message) {
...
void handle_alerts() {
  ...
  if (buttonState != lastButtonState) {
    if (buttonState == LOW) {
      ...
      Serial.println("Send alertmessage to mqtt-broker. Topic /home/smokedetector/alert");
      client.publish("home/smokedetector/alert", "on");
      delay(100);      
      blinkState = true;
    } else {
      Serial.println("Relaycontact: opened (off)");
      Serial.println("Send off-message to mqtt-broker. Topic /home/smokedetector/alert");
      client.publish("home/smokedetector/alert", "off");
      delay(100);
      blinkState = false;
      digitalWrite(LED_BUILTIN, HIGH);
    }
